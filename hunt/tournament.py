        ...
        for match in matches:
            ...
            self.match_results.append({
                "players": [
                    {
                        "name": player_1.name,
                        "score": p1_match_score
                    }, {
                        "name": player_2.name,
                        "score": p2_match_score
                    }
                ]
            })
            # append results of this match to the overall results
            self.results[player_1.name] = self.results.get(player_1.name, 0) + p1_match_score * (0.5 if player_1.name == player_2.name else 1) / len(self.players)
            self.results[player_2.name] = self.results.get(player_2.name, 0) + p2_match_score * (0.5 if player_1.name == player_2.name else 1) / len(self.players)
        return self.results
