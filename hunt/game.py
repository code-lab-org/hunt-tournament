...
class DesignGame(object):
    ...
    def get_payoff(self, my_decision, their_decision):
        ...
        """
        return self.designs[my_decision.design][my_decision.strategy][their_decision.strategy]
    ...
    def play(self, player_1, player_2):
        ...
        if (decision_1.strategy not in [0,1]
                or decision_1.design not in range(len(self.designs))):
            raise InvalidDecisionError(player_1, decision_1)
        decision_2 = player_2.get_decision()
        if (decision_2.strategy not in [0,1]
                or decision_2.design not in range(len(self.designs))):
